...
ArkInventory.Const = { -- constants
	...
	SLOT_SIZE = 28, -- calculated at runtime
	MAX_PET_LEVEL = 25,
	MAX_ACTIVE_PETS = 3,
...
ArkInventory.Const.DatabaseDefaults.global = {
	["option"] = {
		...
		["showdisabled"] = true,
		["restack"] = {
			["cleanup"] = true, -- use blizzard cleanup function -- !!! set back to true
		},
		["bucket"] = {	
			["*"] = nil
		},
...
function ArkInventory.OnLoad( )
	...
	ArkInventory.Global.Location[loc_id].bagCount = #ArkInventory.Global.Location[loc_id].Bags
	...
	table.insert( ArkInventory.Global.Location[loc_id].Bags, REAGENTBANK_CONTAINER )
	ArkInventory.Global.Location[loc_id].tabReagent = #ArkInventory.Global.Location[loc_id].Bags
	ArkInventory.Global.Location[loc_id].bagCount = #ArkInventory.Global.Location[loc_id].Bags
	...
	table.insert( ArkInventory.Global.Location[ArkInventory.Const.Location.Mail].Bags, ArkInventory.Const.Offset.Mail + 2 )
	ArkInventory.Global.Location[loc_id].bagCount = #ArkInventory.Global.Location[loc_id].Bags
...
function ArkInventory.OnEnable( )
	...
	ArkInventory:RegisterEvent( "PLAYERREAGENTBANKSLOTS_CHANGED", "LISTEN_BANK_TAB_REAGENT_UPDATE" ) -- a bag_update event for the reagent bank (-3)
...
function ArkInventory.Frame_Main_Anchor_Set( loc_id, old_scale )
	...
	local f2 = _G[string.format( "%s%s", frame:GetName( ), ArkInventory.Const.Frame.Changer.Name )]
	local f3 = _G[string.format( "%s%s", frame:GetName( ), ArkInventory.Const.Frame.Status.Name )]
	local f4 = _G[string.format( "%s%s", frame:GetName( ), ArkInventory.Const.Frame.Scroll.Name )]
	local f5 = _G[string.format( "%s%s", frame:GetName( ), ArkInventory.Const.Frame.Search.Name )]
...
function ArkInventory.Frame_Container_Draw( frame )
	...
	if ArkInventory.Global.Location[loc_id].drawState <= ArkInventory.Const.Window.Draw.Recalculate then
		...
		for j = 1, ArkInventory.Global.Location[loc_id].maxBar do
			...
			if not barframe then
				...
				--ArkInventory.Output( barframe:GetName( ), " / level = ", barframe:GetFrameLevel( ) )
			end
			...
			if ArkInventory.Global.Location[loc_id].drawState <= ArkInventory.Const.Window.Draw.Recalculate then
				...
				if bar.frame == 1 then
					...
					if anchor == ArkInventory.Const.Anchor.BottomLeft then
						obj:SetPoint( "BOTTOMLEFT", frame, "BOTTOMLEFT", 0, 0 )
					elseif anchor == ArkInventory.Const.Anchor.TopLeft then
						obj:SetPoint( "TOPLEFT", frame, "TOPLEFT", 0, 0 )
					elseif anchor == ArkInventory.Const.Anchor.TopRight then
						obj:SetPoint( "TOPRIGHT", frame, "TOPRIGHT", 0, 0 )
					else -- if anchor == ArkInventory.Const.Anchor.BottomRight then
						obj:SetPoint( "BOTTOMRIGHT", frame, "BOTTOMRIGHT", 0, 0 )
					end
					...
					if bar_index == 1 then
						...
						if anchor == ArkInventory.Const.Anchor.BottomLeft then
							obj:SetPoint( "BOTTOMLEFT", parent, "TOPLEFT", 0, 0 )
						elseif anchor == ArkInventory.Const.Anchor.TopLeft then
							obj:SetPoint( "TOPLEFT", parent, "BOTTOMLEFT", 0, 0 )
						elseif anchor == ArkInventory.Const.Anchor.TopRight then
							obj:SetPoint( "TOPRIGHT", parent, "BOTTOMRIGHT", 0, 0 )
						else -- if anchor == ArkInventory.Const.Anchor.BottomRight then
							obj:SetPoint( "BOTTOMRIGHT", parent, "TOPRIGHT", 0, 0 )
						end
						...
						if anchor == ArkInventory.Const.Anchor.BottomLeft then
							obj:SetPoint( "BOTTOMLEFT", parent, "BOTTOMRIGHT", 0, 0 )
						elseif anchor == ArkInventory.Const.Anchor.TopLeft then
							obj:SetPoint( "TOPLEFT", parent, "TOPRIGHT", 0, 0 )
						elseif anchor == ArkInventory.Const.Anchor.TopRight then
							obj:SetPoint( "TOPRIGHT", parent, "TOPLEFT", 0, 0 )
						else -- if anchor == ArkInventory.Const.Anchor.BottomRight then
							obj:SetPoint( "BOTTOMRIGHT", parent, "BOTTOMLEFT", 0, 0 )
						end
	...
	if ArkInventory.Global.Location[loc_id].drawState <= ArkInventory.Const.Window.Draw.Recalculate then
		...
		for row_index, row in ipairs( c.row ) do
			...
			c.height = c.height + row.height
		
		end
		
		frame:SetWidth( c.width )
		frame:SetHeight( c.height-3 )
...
function ArkInventory.Frame_Item_OnLoad( frame )
	...
	if ArkInventory.Const.SLOT_SIZE then
		frame:SetWidth(ArkInventory.Const.SLOT_SIZE)
		frame:SetHeight(ArkInventory.Const.SLOT_SIZE)
	else
